...
Fixed Issues:
    ...
    * Something with lyrics not syncing LYRICS_WINDOW: "It's full moon again" FLOATING_LYRICS: It&#x27;s full moon again
    * Something causes it to be load the next song slower (like spotify) :(
    * Lyrics Window Auto Scroll for Plain Lyrics doesnt work
    * Turning both lyrics windows off doesnt do anything
    ...
    def set_lyrics(self, lyrics_data: list, current_index: int = -1, highlight: bool = True):
        ...
        for i in range(start, end):
            ...
            if highlight and i == current_index:
                lines.append(f'<div style="font-size:22px; font-weight:700; color:white; margin:8px 0;">{text}</div>')
            else:
                lines.append(f'<div style="font-size:16px; font-weight:500; color:rgba(255,255,255,120); margin:5px 0;">{text}</div>')
